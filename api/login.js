...
module.exports=async function(req,res){
 ...
 try{
  ...
  const rows=await adminRequest(`/rest/v1/erp_profiles?select=id,email,username,full_name,role,active,force_password_change,company_id,branch_id&or=(username.eq.${lookup},email.eq.${lookup})&limit=1`);
  ...
  return json(res,200,{session:{access_token:data.access_token,refresh_token:data.refresh_token},user:{id:profile.id,email:profile.email,user_metadata:{full_name:profile.full_name},companyId:profile.company_id,branchId:profile.branch_id},forcePasswordChange:!!profile.force_password_change});
